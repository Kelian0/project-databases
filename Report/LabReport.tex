...
We can imagine extending our database so that it no longer includes only games from Steam, which focuses exclusively on computer games. Instead, we may want to incorporate titles from additional \texttt{platforms} , such as the \textit{PlayStation 5} or \textit{Nintendo Switch}. \\
