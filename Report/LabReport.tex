...
The Country column will be the foreign key of the countries table.
...
\begin{itemize}
  ...
  \item \textbf{About the game}: A description of the game.
...
The games dataset was cleaned by removing the rows that have missing values for the Developers.
The Developers column was modified to DeveloperID to match the DeveloperID column in the video game studios dataset.
\\
...
appear multiple in rows. The same thing was done for the categories.
...
For this dataset the only modification was to remove the rows where the country was not in 
the world countries dataset.
