...
\noindent
\begin{minipage}{\linewidth}
\noindent\textbf{Bigest Cities with at least 1 studio for each country}
...
\noindent
\begin{minipage}{\linewidth}
\noindent\textbf{Games with the Polish languages supported and the catgory Multi-player and that are not the Adventure game}
...
We can imagine extending our database so that it no longer includes only games from Steam, which focuses exclusively on computer games. Instead, we may want to incorporate titles from additional \texttt{platforms} such as the \textit{PlayStation 5} or the \textit{Nintendo Switch}. \\

To do this, we need to obtain a dataset listing all available platforms and create a dedicated table—called, for example, platform, that stores information about each platform.\\
...
To design the platform table, we assign a serial integer as its primary key and include a non-null name attribute. We may also store additional information such as release date, price, or total sales for each platform. \\
